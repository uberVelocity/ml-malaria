...
def draw_cost(estimations, oob_estimation, acc_estimation,  depth):
    ...
    # ax.plot(depth, color='r' ,label='Maximum Depth')
...
def rand_forest_n_Fold(): 
    ...
    for n_estimationin in estimations: 
        print("##### n estimation",n_estimationin) 
        rf = RandomForestClassifier(warm_start=True,n_estimators=n_estimationin, criterion ="entropy", oob_score=True)
        acc_i, depth_tree= train_random_forest(train_features, test_features, train_labels, test_labels,feature_list,rf)
